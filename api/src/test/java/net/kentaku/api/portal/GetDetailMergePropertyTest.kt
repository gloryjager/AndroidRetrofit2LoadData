...
class GetDetailMergePropertyTest : PortalApiTest() {
    @Test
    fun testMoshi() {
        val json = """
    ...
    fun testGetDetailPropertyTerminal() {
        ...
        propertyList.properties.map {
            it.sameBuildingProperties.map {
                ...
                .plus("invalidid")
                .forEach {
            val url = "/portalapi/callApp.jsp?function=GetDetailMergeProperty&auth_code=218993f4634&entry_date=201708301655&device_type=2&property_full_id=" + it
            println(baseUrl + url)
            val propertyDetailResponse = api.getDetailMergeProperty(url)
                    .subscribeOn(Schedulers.io())
                    .test().await().assertNoErrors().values().first()
