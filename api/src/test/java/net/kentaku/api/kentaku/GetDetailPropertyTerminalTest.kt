...
class GetDetailPropertyTerminalTest : KentakuApiTest() {
    @Test
    fun testMoshi() {
        val json = """
...
"""
    ...
    fun testGetDetailPropertyTerminal() {
        ...
        propertyList.properties.map {
            ...
            val url = "/kentakuapi/callApp.jsp?function=GetDetailPropertyTerminal&auth_code=218993f4634&entry_date=201708301655&device_type=2&property_full_id=" + it
            println(baseUrl + url)
            val propertyDetailResponse = api.getDetailPropertyTerminal(url)
                    .subscribeOn(Schedulers.io())
                    .test().await().assertNoErrors().values().first()
